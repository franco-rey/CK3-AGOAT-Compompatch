...
This aggregate compatibility layer targets CK3 1.19.0.6 / AGOT 0.5.1 and the current 66-mod successor playset. P09–P22 are grounded in runtime evidence from the earlier 49-mod Tybert run. P24–P33, added in the 2026-08-29 revision wave without a version bump, are grounded in the independent 405-year observer run 7899–8304. P35-P44, added in the 2026-08-30 revision wave, are grounded in the 8183 Gar Sagon run, which confirmed the P35/P37 title-name repair (landed_titles 663 MB -> 5.7 MB, guard-book prose 7,918,803 -> 49 lines) and exposed the pending-event flood that P39/P41 address.

P45 comes from the 8296 Lucerys of Driftmark run (22 minutes at 5x, no clock freeze), the first run to verify a wave against live gameplay rather than statically. Eight of the ten patches checked held at exactly zero: P37, P38, P39/P41, P40, P42, P43, P44. P35 fell from 7,907,555 entries to 37, all legitimate chronicle writes.

P46 comes from the 8212 Ser Duncan camp run, the first to reach the guard academy final trials. P47 comes from the 8251 Ormund run (38 minutes, clean exit), which also confirmed P46 at zero and re-verified the earlier wave as holding. That save was healthy throughout: gamestate 200 MB, `landed_titles` 23 KB, pending-event queue empty.

**P24 and P36 remain confirmed NOT working.** Read the P24/P36 GUI note below before touching either. P48 documents a pre-existing override that shipped with no manifest entry or README row until the 2026-08-30 clerical pass; the file itself was not changed.
...
| P46 | AGOT: Royal Guards | Guards the eight academy `add_trait_xp` reads in `agot_guard_positions_update_training_rank_effect`. The effect sets `agot_guard_positions_academy_path_xp` and reads it back on the next eight lines, but `set_variable` is a no-op while CK3 builds a tooltip, so every read hit an unset variable: 2,844 entries (1,422 x 2) in the 8212 Ser Duncan run, reached from `agot_guard_positions.3502`. Zero in prior runs only because nobody had reached the academy final trials. The set-then-read pattern is upstream's, unchanged by P42; each read is now wrapped in `if = { limit = { has_variable = agot_guard_positions_academy_path_xp } ... }`, skipped in tooltip mode and always taken at execution time. Confirmed 0 in the 8251 Ormund run. |
| P47 | AGOT: Royal Guards | Moves the guard HUD widget from `position = { -631 0 }` to `{ -960 0 }`. All bottom-right anchored widgets extend leftward from their offset, so at 114 px wide the guard row spanned 631 to 745 and sat on top of Advanced Character Search's "find person" button (630 to 685) and Search and Trade Artifacts' right "find artifact" button (690 to 746), hiding both. -960 continues S&T's own 90 px pitch and leaves the same 34 px gap it uses between its own buttons; new span 960 to 1074. Nothing else in the playset anchors bottom-right beyond 926 and vanilla's `timeline_widget` occupies only 0 to 649. Size and all three button bindings unchanged. |
| P48 | Active Courtiers | Neutralises two `fp3_character_uninvolved_in_struggle_trigger` calls in `accou_marriage_candidate_quality_value`, replacing each with `always = no`. AGOT disables the whole FP3 layer and comments the trigger out of its `06_fp3_scripted_triggers.txt`, so both AND branches are Persia-struggle-only and can never be true in AGOT. **Pre-existing override that shipped without a manifest entry or README row**; documented retroactively in the 2026-08-30 clerical pass. The file itself was not changed. |
