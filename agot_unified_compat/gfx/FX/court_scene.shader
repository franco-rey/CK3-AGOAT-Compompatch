...
#
# Battle Graphics AGOT Compatibility Patch (3235061780, load pos 78) overwrote this
# file with a version built against AGOT alone. AGOT Color Picker for Clothes
# (3381385128, load pos 17) owns gfx/FX/agot_portrait_decals_shared.fxh, which
# defines ApplyVariationPatterns with SEVEN parameters - the 7th is PortraitEffect.
# The BG patch's court_scene.shader calls it with SIX. Result:
#     shadercache/dx11/ps_5_0/*.scache: error X3013:
#         ApplyVariationPatterns - no matching 6 parameter function
#     gfx_dx11_shaderstate.cpp:165: Failed getting shader for PS_attachment
# and then, for every garment mesh in the game:
...
# garment received a material: characters rendered nude with distorted limbs.
#
# Fix: this file is AGOT Color Picker for Clothes' court_scene.shader VERBATIM.
# It threads PortraitEffect through CommonPixelShaderColor, CommonPixelShader and
# CommonPixelShaderWithTwoNormal, ~10 AGOT_ScanMarkerDecals call sites,
# AddDragonDecals and AGOT_TryApplyStatueEffect - 44 lines that cannot simply be
# grafted onto the BG version. Battle Graphics' ACG Addition block is appended;
# that block is its ONLY unique contribution to this file, and its two Effects are
# referenced by acg_MESH_flag_banner.asset and acg_MESH_flag_weapon.asset.
#
...
Includes = {
	...
	# END MOD
	# MOD(agot) baked-mesh particle cards
	"agot_particle_card.fxh"
	# END MOD
}
	...
	MainCode VS_particle_card
	{
		...
		[[
			PDX_MAIN
			{
				...
				uint JointIndex = Input.BoneIndex[0] + Input.InstanceIndices.x;
				SParticleCard Card = PdxReadParticleCard( JointIndex );

				// Plain constants here: this file includes vanilla cw/camera.fxh,
				// so nothing rebuilds the camera and the name costs nothing.
				float3 CardNormal;
				float3 LocalOffset = PdxParticleCornerOffset( Card, ViewMatrix, Input.Position.xy, CardNormal );

				float4x4 World = PdxMeshGetWorldMatrix( Input.InstanceIndices.y );
				float4 WorldOrigin = mul( World, float4( Card._Origin, 1.0f ) );
				float3 OriginWS = WorldOrigin.xyz / max( WorldOrigin.w, 1e-6f );

				// Measured with a probe vector - see jomini/portrait.shader.
				float4 WorldProbe = mul( World, float4( Card._Origin + float3( 1.0f, 0.0f, 0.0f ), 1.0f ) );
				float WorldScale = length( WorldProbe.xyz / max( WorldProbe.w, 1e-6f ) - OriginWS );

				float3 WorldSpacePos = OriginWS + LocalOffset * WorldScale;

				Out.WorldSpacePos = WorldSpacePos;
				Out.Position = FixProjectionAndMul( ViewProjectionMatrix, float4( WorldSpacePos, 1.0f ) );

				float Opacity = Card._Opacity * PARTICLE_OPACITY_MULT;
			#ifdef PARTICLE_PARENT_SCALE_OPACITY
				// Kept in step with jomini/portrait.shader - see the note there.
				Opacity *= PdxParticleParentScale( World, Card._Origin, OriginWS );
			#endif

				Out.UV0 = PdxParticleAtlasUV( Input.UV0, 0.0f );
				Out.UV1 = Out.UV0;
				Out.UV2 = float2( 0.0f, saturate( Opacity ) );

				// PS_court_selection reads only InstanceIndex. These need any
				// value that cannot be a NaN.
				Out.Normal = normalize( CameraPosition - WorldSpacePos );
				Out.Tangent = CameraRightDir;
				Out.Bitangent = CameraUpDir;

				// No ShadowProj: the VS_OUTPUT_PDXMESHPORTRAIT in this file is
				// the shorter court-scene variant and does not declare one.
				// Everything it DOES declare is written below, or the struct
				// interpolates undefined values.
				Out.InstanceIndex = Input.InstanceIndices.y;
				Out.AGOT_IsDynamicTerrainLoaded = 1;
				Out.AGOT_SmuggledWeights = float3( 0.0f, 0.0f, 0.0f );
				Out.AGOT_DecayDiffuseIndex = 0;
...
PixelShader =
{
	...
	[[
		...
		float3 CommonPixelShaderColor( float4 Diffuse, float4 Properties, float3 Normal, in VS_OUTPUT_PDXMESHPORTRAIT Input, float HoverMult, in AGOT_SPortraitEffect PortraitEffect )
		{
			GetSpecularAA( Normal, 1.0f, 1.0f, Properties.a );

			// MOD
			AGOT_TryApplyStatueEffect(PortraitEffect, Diffuse, Properties);
			// END MOD
		...
		float3 CommonPixelShader( float4 Diffuse, float4 Properties, float3 NormalSample, in VS_OUTPUT_PDXMESHPORTRAIT Input, float HoverMult, in AGOT_SPortraitEffect PortraitEffect )
		{
		...
		float3 CommonPixelShaderWithTwoNormal( float4 Diffuse, float4 Properties, float3 FirstNormalSample, float3 SecondNormalSample, float NormalUVChannel, in VS_OUTPUT_PDXMESHPORTRAIT Input, float HoverMult, in AGOT_SPortraitEffect PortraitEffect )
		{
	...
	MainCode PS_skin
	{
		...
		[[
			PDX_MAIN
			{
				...
				// MOD
				AGOT_SPortraitEffect PortraitEffect = AGOT_ScanMarkerDecals(DecalCount, false, true);
				// END MOD
	...
	MainCode PS_eye
	{
		...
		[[
			PDX_MAIN
			{
				...
				// MOD
				AGOT_SPortraitEffect PortraitEffect = AGOT_ScanMarkerDecals(DecalCount, false, false);
				// END MOD
	...
	MainCode PS_attachment
	{
		...
		[[
			PDX_MAIN
			{
				...
				// MOD
				AGOT_SPortraitEffect PortraitEffect = AGOT_ScanMarkerDecals(DecalCount, true, false);
				// END MOD
					...
					ApplyVariationPatterns( Input, Diffuse, Properties, PatternNormal, SecondColorMask, NormalUVChannel, PortraitEffect);
	...
	MainCode PS_dragon
	{
		...
		[[
			PDX_MAIN
			{
				...
				#ifdef DECALS
					AddDragonDecals( Diffuse, NormalSample, Properties, ColorMask, UV0, UV1, Input.InstanceIndex, PortraitEffect );
				#endif
	...
	MainCode PS_court
	{
		...
		[[
			...
			PDX_MAIN
			{
                ...
                AGOT_SPortraitEffect PortraitEffect = AGOT_GetDefaultPortraitEffect();
				float3 Color = CommonPixelShader( Diffuse, Properties, NormalSample, Input, HoverMult, PortraitEffect );
	...
	# MOD(agot) baked-mesh particle cards
	#
	# Twin of PS_particle_card in jomini/portrait.shader. Keep the two in step.
	MainCode PS_particle_card
	{
		Input = "VS_OUTPUT_PDXMESHPORTRAIT"
		Output = "PS_COLOR_SSAO"
		Code
		[[
			PDX_MAIN
			{
				...
				// UV2.y holds the opacity from the bone Z scale.
				float Alpha = Diffuse.a * Input.UV2.y;

				if ( Alpha <= PARTICLE_ALPHA_CLIP )
				{
					discard;
				}

				PS_COLOR_SSAO Out;
				Out.Color = float4( Diffuse.rgb * PARTICLE_TINT, Alpha );
...
# MOD(agot) baked-mesh particle cards - see jomini/portrait.shader for why
BlendState particle_card_alpha
{
	BlendEnable = yes
...
DepthStencilState particle_card
{
	DepthEnable = yes
	DepthWriteEnable = no
}
...
#MOD-AGOT baked-mesh particle cards
#
# Every effect in jomini/portrait.shader needs a match here, as <name> and
# <name>_selection, plus <name>_shadow if one is ever added. The engine hardcodes
# this file. A missing base effect reports:
#   Failed to create material with shader portrait_particle_card
#   (in gfx/FX/court_scene.shader)
# A missing _selection twin reports nothing and the mesh never draws.
# Declared without PARTICLE_DRAGON_PALETTE, so the cards render untinted.
# Dragons do not appear in court scenes, so the scan would find no palette.
Effect portrait_particle_card_dragon
{
	VertexShader = "VS_particle_card"
	PixelShader = "PS_particle_card"
	BlendState = "particle_card_alpha"
	DepthStencilState = "particle_card"
	RasterizerState = "rasterizer_no_culling"
	Defines = { "PARTICLE_BILLBOARD" }
}

Effect portrait_particle_card_dragon_selection
{
	VertexShader = "VS_particle_card"
	PixelShader = "PS_court_selection"
	Defines = { "PARTICLE_BILLBOARD" }
...
# Twin of the lit effect. Declared WITHOUT PARTICLE_LIT and without
# PARTICLE_DRAGON_DARK_GATE, for the same reason the dragon one drops the
# palette: the gate would scan for a palette that is not there and then hide
# every card. Compiling it here would also drag the whole decal-buffer walk into
# this file's vertex shader for nothing.
Effect portrait_particle_card_dragon_lit
{
	VertexShader = "VS_particle_card"
	PixelShader = "PS_particle_card"
	BlendState = "particle_card_alpha"
...
Effect portrait_particle_card_dragon_lit_selection
{
	VertexShader = "VS_particle_card"
	PixelShader = "PS_court_selection"
	Defines = { "PARTICLE_BILLBOARD" }
