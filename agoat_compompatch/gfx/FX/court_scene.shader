...
ConstantBuffer( SceneShadowConstants )
{
	...
	float4 _ProjectionFactors_ShadowType[4];
	float4 _AtlasOffsetAndScale[4*6];
	float4 _ShadowFade;
	uint _NumShadows;
...
PixelShader =
{
	...
	[[
		...
		float CalcShadow( float3 WorldSpacePos, int ShadowIndex )
		{
			...
			float3 FadeFactor = saturate( float3( ( 1.0 - abs( 0.5 - ShadowProj.xy ) * 2.0 ) * 32.0, CalcDepthFadeFactor( ShadowProj.z, ShadowIndex ) ) );
